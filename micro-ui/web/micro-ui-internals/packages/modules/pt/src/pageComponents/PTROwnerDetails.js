    ...
    <React.Fragment>
      <div style={{ marginBottom: "16px" }}>
        ...
        <div style={{ border: "1px solid #E3E3E3", padding: "16px", marginTop: "8px" }}>
          ...
          <LabelFieldPair>
            <CardLabel className="card-label-smaller">{t("PTR_ADHAR_NUMBER") + " *"}</CardLabel>
            <div className="field">
              <Controller
                control={control}
                name={"adharNumber"}
                defaultValue={owner?.adharNumber}
                rules={{
                 // required: t("CORE_COMMON_REQUIRED_ERRMSG"),
                  validate: {
                    pattern: (val) => (/^\d{12}$/.test(val) ? true : t("ERR_AADHAR_NUMBER_INVALID")),
                  },
                    // check the validatin for the adhar number as i just googlr it accordingly 
                }}
                render={(props) => (
                  <TextInput
                    value={props.value}
                    ...
                    onChange={(e) => {
                      props.onChange(e);
                      setFocusIndex({ index: owner.key, type: "adharNumber" });
                    }}
          ...
          <CardLabelError style={errorStyle}>{localFormState.touched.adharNumber ? errors?.adharNumber?.message : ""}</CardLabelError>
