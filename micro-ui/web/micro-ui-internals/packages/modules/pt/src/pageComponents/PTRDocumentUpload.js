...
const PTRDocumentUpload = ({ t, config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
  ...
  const { isLoading, data } = Digit.Hooks.ptr.usePetMDMS(stateId, "PetService", "PetDocuments");   


  const mutationDocs = data?.PetService?.MutationDocuments;
  const commonDocs = data?.PetService?.PetDocuments;

  const PTRDocument = isMutation
    ? mutationDocs?.map?.((doc) => commonDocs.find((e) => doc.code === e.code) || doc)
    : data?.PetService?.PetDocuments;
    console.log("vhvhvmvmh",PTRDocument)
  const goNext = () => {
  ...
  return (
    <div>
      ...
      {PTRDocument?.map((document, index) => {
        ...
        //   }
        // }
        //if (document.code === "OWNER.IDENTITYPROOF" || document.code === "OWNER.IDENTITYPROOF" ){
